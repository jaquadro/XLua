...
PROPS_DATA_START()
	...
	PropData_Group(PROPID_OFFSET, PSTR("Offset"), PSTR("...")),
	PropData_EditNumber(PROPID_OFFSETX, PSTR("X"), PSTR("...")),
	PropData_EditNumber(PROPID_OFFSETY, PSTR("Y"), PSTR("...")),

	PropData_Group(PROPID_DISPLAY, PSTR("Display"), PSTR("...")),
	PropData_Filename(PROPID_FILENAME, PSTR("Image"), PSTR("..."), FALSE),

PROPS_DATA_END()

PropData TransProps[] = {
	PropData_Group(PROPID_TRANSGROUP, PSTR("Transparent Color"), PSTR("...")),
	PropData_CheckBox(PROPID_TRANSFIRST, PSTR("First Pixel"), PSTR("...")),
	PropData_Color_Check(PROPID_TRANSCOLOR, PSTR("Color"), PSTR("...")),
	PropData_End()
};
...
BOOL WINAPI DLLExport GetProperties(LPMV mV, LPEDATA edPtr, BOOL bMasterItem)
{
	...
	mvInsertProps(mV, edPtr, TransProps, PROPID_TAB_DISPLAY, TRUE);
#endif // !RUN_ONLY
...
BOOL WINAPI EditObject (mv _far *mV, fpObjInfo oiPtr, fpLevObj loPtr, LPEDATA edPtr)
{
	...
	strcpy(szFileName, edPtr->filename);
	...
	if ( ChoosePicture(&ofn, TRUE, pImgMgr, NULL, PICSEL_IMAGES) )
	{
		...
		{
			//edPtr->cWidth = pFilter.GetWidth();
			//edPtr->cHeight = pFilter.GetHeight();
			...
			// Copy new filename
			strcpy(edPtr->filename, szFileName);
...
{
#ifndef RUN_ONLY
	rc->right = rc->left + edPtr->width;	// edPtr->swidth;
	rc->bottom = rc->top + edPtr->height;	// edPtr->sheight;
#endif // !RUN_ONLY
	return;
}
...
void WINAPI DLLExport EditorDisplay(mv _far *mV, fpObjInfo oiPtr, fpLevObj loPtr, LPEDATA edPtr, RECT FAR *rc)
{
	...
	// Import picture file
	//if ( edPtr == NULL || *edPtr->filename == 0 || proto == NULL || pImgMgr == NULL ||
	//	!ImportImage(pImgMgr, edPtr->filename, &sf, NULL, IMPORT_IMAGE_USESURFACEDEPTH | IMPORT_IMAGE_USESURFACEPALETTE) ) 
	//{
	...
	/*else {
		...
		if ( (oiPtr->oiHdr.oiInkEffect & EFFECTFLAG_TRANSPARENT) != 0 ) {
			...
			// check if the transparent color was set by the ImportImage function (for example if GIF image with transparent color)
			if ( sf.GetTransparentColor() == RGB(0,0,0) ) {
				COLORREF trspColor = edPtr->dwTransColor;
				if ( (edPtr->dwOptions & 1) != 0 ) {
		...
		sf.Blit(*wSurf, rc->left, rc->top, edPtr->cxOrigin, edPtr->cyOrigin, edPtr->cWidth, edPtr->cHeight,
				bm, bo, effectParam, bAntiA ? BLTF_ANTIA : 0);
	}*/
